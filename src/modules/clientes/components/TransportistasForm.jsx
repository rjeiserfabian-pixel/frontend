...
const TransportistasForm = ({ open, onClose, onSuccess, transportistaEdit }) => {
  const { guardarTransportista, consultarDocumento } = useTransportistas();
  const [isSearchingDni, setIsSearchingDni] = useState(false);

  const { control, handleSubmit, reset, setValue, getValues, formState: { errors, isSubmitting }, watch } = useForm({
    resolver: zodResolver(schema),
    defaultValues: {
      tipo_documento: 'RUC', numero_documento: '', nombre_o_razon_social: '', telefono: '', direccion: '', email: '', licencia_conducir: '', categoria_licencia: ''
    }
  });
  ...
  useEffect(() => {
    if (transportistaEdit) {
      reset({
         ...transportistaEdit,
         email: transportistaEdit.email || '',
         licencia_conducir: transportistaEdit.licencia_conducir || '',
         categoria_licencia: transportistaEdit.categoria_licencia || ''
      });
    } else {
      reset({ tipo_documento: 'RUC', numero_documento: '', nombre_o_razon_social: '', telefono: '', direccion: '', email: '', licencia_conducir: '', categoria_licencia: '' });
    }
  }, [transportistaEdit, open, reset]);
  ...
  const onSubmit = async (data) => {
    const exito = await guardarTransportista(data, transportistaEdit?.id);
    if (exito) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {transportistaEdit ? 'Editar Transportista' : 'Nuevo Transportista'}
            ...
            <Grid item xs={12} sm={8}>
              <Box sx={{ display: 'flex', gap: 1 }}>
                ...
                <Button 
                  ...
                  disabled={isSearchingDni || isSubmitting || transportistaEdit != null}
                  sx={{ minWidth: '120px' }}
                >
              ...
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    ...
                    fullWidth
                    size="small"
                    error={!!errors.licencia_conducir}
                    helperText={errors.licencia_conducir?.message}
                  />
                )}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <Controller
                ...
                render={({ field }) => (
                  <TextField
                    {...field}
                    label="Categoría Licencia (Ej. A-IIb)"
                    fullWidth
                    size="small"
                    error={!!errors.categoria_licencia}
                    helperText={errors.categoria_licencia?.message}
                  />
                )}
              />
            </Grid>
...
export default React.memo(TransportistasForm);
