...
const nivelStockSx = (nivel) => {
  if (nivel === 'bajo') return { bgcolor: '#fef2f2', color: '#b91c1c', border: '1px solid #fecaca' };
  if (nivel === 'ok') return { bgcolor: '#f0fdf4', color: '#166534', border: '1px solid #dcfce7' };
  return { bgcolor: '#f8fafc', color: '#64748b', border: '1px solid', borderColor: 'divider' };
};
      ...
      <TableRow hover sx={{ '& td': { borderColor: 'divider' } }}>
        <TableCell sx={{ py: 1.5 }}>
          <IconButton
            ...
            sx={{ bgcolor: abierto ? '#f1f5f9' : 'transparent', borderRadius: '8px' }}
          >
            {abierto ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </IconButton>
        </TableCell>
        <TableCell sx={{ fontFamily: 'monospace', color: 'text.secondary', fontWeight: 600 }}>{repuesto.codigo}</TableCell>
        <TableCell sx={{ fontWeight: 700, color: '#1e293b' }}>{repuesto.nombre}</TableCell>
        <TableCell>
          {repuesto.categoria_nombre
            ? <Chip label={repuesto.categoria_nombre} size="small" sx={{ bgcolor: '#f1f5f9', color: '#475569', fontWeight: 600, borderRadius: '6px' }} />
      ...
      <TableRow>
        <TableCell colSpan={9} sx={{ p: 0, borderBottom: abierto ? undefined : 'none' }}>
          <Collapse in={abierto} timeout="auto" unmountOnExit>
            <Box sx={{ p: 2.5, bgcolor: '#f8fafc', borderTop: '1px dashed', borderColor: 'divider' }}>
              ...
              <Paper elevation={0} sx={{ borderRadius: '12px', border: '1px solid', borderColor: 'divider', overflow: 'hidden' }}>
                <Table size="small">
                  <TableHead>
                    <TableRow sx={{ '& th': { bgcolor: 'white', fontWeight: 700, color: 'text.secondary', borderColor: 'divider' } }}>
                  ...
                  <TableBody>
                    {ubicaciones.map(u => {
                      ...
                      return (
                        <TableRow key={u.id} sx={{ bgcolor: 'white', '&:last-child td': { borderBottom: 0 } }}>
                              ...
                              <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                                {segmentosUbicacion(u.ubicacion_detalle).map((seg, i) => (
                                  <Chip
                                    ...
                                    label={seg}
                                    size="small"
                                    sx={{ bgcolor: '#eef2ff', color: '#3730a3', fontWeight: 600, borderRadius: '6px', height: '22px', fontSize: '0.72rem' }}
                                  />
                                ))}
                              </Box>
                            ) : (
...
export default function StockUbicacionesPage() {
  ...
  return (
    <Box sx={{ maxWidth: '1400px', mx: 'auto', pb: 6 }}>
      {/* Encabezado */}
      <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: '20px', border: '1px solid', borderColor: 'divider', background: 'linear-gradient(to right bottom, #ffffff, #f8fafc)' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ width: 48, height: 48, borderRadius: '14px', bgcolor: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <MapPin size={22} color="white" />
            </Box>
            <Box>
              <Typography variant="h5" fontWeight="800" color="#0f172a">Ubicaciones de Stock</Typography>
          ...
          <Chip
            icon={<MapPin size={14} color="#1d4ed8" />}
            label={<span>Sucursal: <strong>{sucursalActiva?.nombre || '—'}</strong> (solo esta sucursal)</span>}
            sx={{ bgcolor: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe', fontWeight: 600, borderRadius: '10px', py: 2.2 }}
          />
        </Box>
      </Paper>

      {/* Filtros */}
      <Paper elevation={0} sx={{ p: 2.5, mb: 3, borderRadius: '20px', border: '1px solid', borderColor: 'divider', display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        <TextField
      ...
      {/* Tabla */}
      <Paper elevation={0} sx={{ width: '100%', overflow: 'hidden', borderRadius: '20px', border: '1px solid', borderColor: 'divider' }}>
        {loading ? (
          ...
          <TableContainer>
            <Table>
              <TableHead>
                <TableRow sx={{ '& th': { bgcolor: '#f8fafc', fontWeight: 700, color: 'text.secondary', py: 2, borderColor: 'divider' } }}>
                  <TableCell width={44} />
